...
# Password: DEMO_SEED_PASSWORD in request body when server env lacks it (never printed).
#
...
if [[ "$MODE" == "--api" ]]; then
  ...
  # Try existing demo owner login first (re-seed).
...
PY
  ...
  # Bootstrap: register a temporary owner (staging allows unverified login).
