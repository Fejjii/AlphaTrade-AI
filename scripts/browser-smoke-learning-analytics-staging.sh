#!/usr/bin/env bash
# Read-only browser smoke for staging learning analytics (Slice 84).
# No orders, proposals, approvals, execution, exchange, Telegram, or automation.
#
# Usage:
#   STAGING_BOOTSTRAP_PASSWORD='...' ./scripts/browser-smoke-learning-analytics-staging.sh
#
# Optional overrides:
...
export PLAYWRIGHT_STAGING_LEARNING_ANALYTICS_SMOKE=1
...
if [[ -z "${STAGING_BOOTSTRAP_PASSWORD:-}" ]]; then
  cat >&2 <<'EOF'
STAGING_BOOTSTRAP_PASSWORD required for learning analytics browser smoke (not printed).
...
# learning-analytics-staging.spec.ts reads STAGING_DEMO_* env vars.
...
echo "Browser smoke — ${PLAYWRIGHT_BASE_URL}/learning-analytics (read only)"
npx playwright test e2e/learning-analytics-staging.spec.ts --project=chromium
echo "Learning analytics browser smoke passed."
