#!/usr/bin/env bash
# Read-only browser smoke for staging lessons (Slice 90B).
# No orders, proposals, approvals, execution, exchange, Telegram, or automation.
#
# Usage:
#   STAGING_BOOTSTRAP_PASSWORD='...' ./scripts/browser-smoke-lessons-staging.sh
set -euo pipefail
...
export PLAYWRIGHT_STAGING_LESSONS_SMOKE=1
...
if [[ -z "${STAGING_BOOTSTRAP_PASSWORD:-}" ]]; then
  cat >&2 <<'EOF'
STAGING_BOOTSTRAP_PASSWORD required for lessons browser smoke (not printed).
...
echo "Browser smoke — ${PLAYWRIGHT_BASE_URL}/lessons (read only)"
npx playwright test e2e/lessons-staging.spec.ts --project=chromium
echo "Lessons browser smoke passed."
