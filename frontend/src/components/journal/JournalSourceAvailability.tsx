...
export type JournalSourceStatus = {
  name: string;
  available: boolean;
  error?: string | null;
...
type JournalSourceAvailabilityProps = {
  sources: JournalSourceStatus[];
  onRetry?: () => void;
  limitations?: string[];
};

export function JournalSourceAvailability({
  sources,
  onRetry,
  limitations = [],
}: JournalSourceAvailabilityProps) {
  ...
  return (
    <section
      aria-labelledby="journal-source-availability-heading"
      data-testid="journal-source-availability"
      className="space-y-3"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h2
            id="journal-source-availability-heading"
            className="text-lg font-semibold text-text-primary"
          >
            Journal limitations and source availability
          </h2>
      ...
      {allFailed ? (
        <div
          role="alert"
          data-testid="journal-sources-all-failed"
          className="rounded-control border border-danger-border bg-danger-muted/40 px-3 py-2 text-sm text-danger"
        >
          All Journal sources are unavailable. Counts and empty states are suppressed.
        </div>
      ) : null}
      ...
      {partial ? (
        <div
          role="status"
          data-testid="journal-sources-partial"
      ...
      {limitations.length > 0 ? (
        <div
          role="status"
          data-testid="journal-limitations"
      ...
      <ul className="grid gap-2 sm:grid-cols-2" data-testid="journal-source-list">
        {sources.map((source) => (
          <li
            key={source.name}
            className="rounded-control border border-border-subtle px-3 py-2 text-sm"
            data-testid={`journal-source-${source.name.toLowerCase().replaceAll(" ", "-")}`}
          >
