...
type LessonsSourceAvailabilityProps = {
  sources: LessonsSourceStatus[];
  onRetry?: () => void;
  limitations?: string[];
};

export function LessonsSourceAvailability({
  sources,
  onRetry,
  limitations = [],
}: LessonsSourceAvailabilityProps) {
  ...
  return (
    <section
      aria-labelledby="lessons-source-availability-heading"
      data-testid="lessons-source-availability"
      className="space-y-3"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h2
            id="lessons-source-availability-heading"
            className="text-lg font-semibold text-text-primary"
          >
            Source availability and limitations
          </h2>
          <p className="mt-1 text-sm text-text-muted">
            Failed lesson sources never appear as empty review queues. Unavailable counts stay
            hidden.
          </p>
        </div>
      ...
      {allFailed ? (
        <div
          role="alert"
          data-testid="lessons-sources-all-failed"
          className="rounded-control border border-danger-border bg-danger-muted/40 px-3 py-2 text-sm text-danger"
        >
          All lesson sources are unavailable. Counts and empty states are suppressed.
        </div>
      ) : null}
      ...
      {partial ? (
        <div
          role="status"
          data-testid="lessons-sources-partial"
      ...
      {limitations.length > 0 ? (
        <div
          role="status"
          data-testid="lessons-limitations"
      ...
      <ul className="grid gap-2 sm:grid-cols-2" data-testid="lessons-source-list">
        {sources.map((source) => (
          <li
            key={source.name}
            className="rounded-control border border-border-subtle px-3 py-2 text-sm"
            data-testid={`lessons-source-${source.name.toLowerCase().replaceAll(" ", "-")}`}
          >
