"""Slice 80 — paper validation candidate queue endpoints."""
...
ORG_A = uuid.UUID("00000000-0000-0000-0000-000000008001")
ORG_B = uuid.UUID("00000000-0000-0000-0000-000000008002")
USER_A = uuid.UUID("00000000-0000-0000-0000-000000008011")
USER_B = uuid.UUID("00000000-0000-0000-0000-000000008012")
...
_BASE = {
    ...
    "jwt_secret": "paper-validation-candidate-secret-32",
...
def client() -> Iterator[tuple[TestClient, sessionmaker[Session]]]:
    ...
    with factory() as session:
        session.add(Organization(id=ORG_A, name="Candidate Org A"))
        session.add(Organization(id=ORG_B, name="Candidate Org B"))
        session.add(
            User(
                id=USER_A,
                email="candidate-a@test.example",
        ...
        session.add(
            User(
                id=USER_B,
                email="candidate-b@test.example",
                hashed_password=hash_password("SecurePass123!", settings),
                email_verified=True,
            )
        )
...
def _create_draft_with_prep(
    client: TestClient,
    headers: dict[str, str],
    factory: sessionmaker[Session],
    *,
    prep_overrides: dict[str, object] | None = None,
    ...
    created = client.post(
        ...
        json={
            ...
            "notes": "Candidate queue test",
            "risk_mode": "conservative",
        },
    )
    assert created.status_code == 200, created.text
    draft_id = created.json()["draft"]["draft_id"]
    prep_payload = dict(_READY_PREP)
    if prep_overrides:
        prep_payload.update(prep_overrides)
    prep = client.patch(
    ...
    if require_ready:
        assert prep.json()["is_ready_for_validation"] is True
    return draft_id
...
def _create_ready_draft(
    ...
    *,
    ...
    return _create_draft_with_prep(
        client,
        headers,
        factory,
...
def _queue_payload(**overrides: object) -> dict[str, object]:
    payload = {"confirm": QUEUE_PAPER_VALIDATION_CANDIDATE_CONFIRM}
...
def test_exact_confirmation_required(
    ...
    headers = _auth(test_client, "candidate-a@test.example")
    draft_id = _create_ready_draft(test_client, headers, factory)

    response = test_client.post(
        f"/paper-validation/drafts/{draft_id}/queue",
        headers=headers,
        json=_queue_payload(confirm="WRONG"),
    )
    assert response.status_code == 422
    assert "confirmation required" in response.json()["error"]["message"].lower()


def test_cannot_queue_draft_not_ready(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    draft_id = _create_draft_with_prep(
        test_client,
        headers,
        factory,
        prep_overrides={
...
def test_cannot_queue_needs_review_draft(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    draft_id = _create_draft_with_prep(
        ...
        headers,
        factory,
        prep_overrides={"prep_status": "needs_review"},
        require_ready=False,
    )
    ...
    assert response.status_code == 422
    assert "ready_for_validation" in response.json()["error"]["message"].lower()


def test_cannot_queue_archived_draft(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    ...
    with factory() as session:
        ...
        assert row is not None
        row.status = PaperValidationDraftStatus.ARCHIVED.value
        session.commit()
...
def test_can_queue_ready_for_validation_draft(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    ...
    assert body["candidate"]["draft_id"] == draft_id
    assert body["candidate"]["candidate_status"] == "queued"
    assert body["candidate"]["thesis"] == _READY_PREP["thesis"]


def test_duplicate_queue_returns_existing(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
...
def test_candidate_list_and_read_tenant_scoped(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers_a = _auth(test_client, "candidate-a@test.example")
    headers_b = _auth(test_client, "candidate-b@test.example")
    draft_id = _create_ready_draft(test_client, headers_a, factory)
    ...
    listing = test_client.get("/paper-validation/candidates", headers=headers_a)
    assert listing.status_code == 200
    assert listing.json()["total"] == 1

    read = test_client.get(f"/paper-validation/candidates/{candidate_id}", headers=headers_a)
    assert read.status_code == 200
    assert read.json()["symbol"] == "BTCUSDT"

    other_list = test_client.get("/paper-validation/candidates", headers=headers_b)
    assert other_list.status_code == 200
    assert other_list.json()["total"] == 0

    other_read = test_client.get(
...
def test_candidate_summary_works(client: tuple[TestClient, sessionmaker[Session]]) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    draft_id = _create_ready_draft(test_client, headers, factory)
    test_client.post(
    ...
    summary = test_client.get("/paper-validation/candidates/summary", headers=headers)
...
def test_candidate_status_update_works(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    ...
    assert updated.json()["candidate_status"] == "reviewing"
...
def test_invalid_candidate_status_rejected(
    ...
    response = test_client.patch(
        f"/paper-validation/candidates/{candidate_id}",
        headers=headers,
        json={"candidate_status": "executing"},
    )
    assert response.status_code == 422


def test_queue_emits_audit_events(client: tuple[TestClient, sessionmaker[Session]]) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    draft_id = _create_ready_draft(test_client, headers, factory)
    test_client.post(
    ...
    assert "paper_validation_candidate_queue_requested" in actions
    assert "paper_validation_candidate_created" in actions
...
def test_queue_does_not_execute_or_deliver(
...
def test_no_candidate_rows_duplicated_on_repeat_queue(
    client: tuple[TestClient, sessionmaker[Session]],
) -> None:
    test_client, factory = client
    headers = _auth(test_client, "candidate-a@test.example")
    ...
    with factory() as session:
        rows = list(session.scalars(select(PaperValidationCandidate)).all())
        assert len(rows) == 1
