...
class CandidateLifecycleService:
    ...
    def transition(
        ...
        *,
        ...
        """Apply a descendant transition. Identical replays are idempotent."""
        current = self._repository.get_by_id(organization_id, candidate_id)
        if current is None:
        ...
        return self._repository.commit_transition(
            organization_id=organization_id,
            candidate_id=candidate_id,
            expected_version=current.transition_version,
            new_state=new_state,
            previous_state=current.state,
