...
logger = structlog.get_logger("telegram_activation.cli")
...
def run_telegram_paper_process(*, once: bool = False) -> str:
    """Drain and poll when armed. Idle when disarmed. Does not trade.

    Disarmed startup constructs Settings and returns ``disarmed`` without
    opening PostgreSQL, Redis, or the Telegram network. Armed startup keeps
    global secret and dependency validation.
    """
    ...
    settings = load_worker_process_settings(WorkerBootRole.TELEGRAM_PAPER)
    ...
    if settings_are_disarmed_paper_worker(settings):
        ...
        if not once:
            idle_disarmed_until_signal(settings, component="telegram_paper")
        return "disarmed"
...
def _run(*, once: bool) -> int:
    """Paper Telegram process. Idle when disarmed. Does not trade."""

    run_telegram_paper_process(once=once)
    return 0
